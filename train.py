...
dataset = CRISDataset(FLAGS.data_path, vocab_path, 5000)
X, Y = dataset.load()
...
embedding_path = os.path.join(FLAGS.data_path, "embeddings.npy")
embedding = data_helpers.load_embeddings(embedding_path, vocab_size, 200)
...
def train(x_train, y_train, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs/d", timestamp))
            print("Writing to {}\n".format(out_dir))
...
def main(argv=None):
    x_train, x_dev, y_train, y_dev = train_test_split(x_shuffled, y_shuffled, test_size=0.2, random_state=42, stratify=y_shuffled)
    y_train = np.eye(2)[y_train]
    y_dev = np.eye(2)[y_dev]
